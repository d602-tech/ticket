...
export const StatCard: React.FC<StatCardProps> = ({ title, value, icon: Icon, colorClass, isCurrency = false }) => {
  ...
  return (
    <div className={`relative overflow-hidden bg-white p-6 rounded-2xl border border-slate-100 shadow-[0_4px_20px_-4px_rgba(0,0,0,0.05)] hover:shadow-[0_8px_30px_-4px_rgba(0,0,0,0.1)] transition-all duration-300 hover:scale-[1.02] group`}>
      ...
      <div className="relative z-10 flex flex-col h-full justify-between">
        <div className="flex justify-between items-start mb-4">
          <div className={`p-3 rounded-xl ${colorClass} bg-opacity-10 text-${colorName}-600 group-hover:bg-opacity-20 transition-all shadow-sm`}>
          ...
          {isCurrency && (
            <span className="text-[10px] font-bold tracking-wider text-slate-400 bg-slate-50 border border-slate-100 px-2 py-1 rounded-full uppercase">
              TWD
            </span>
          )}
        </div>
        ...
        <div className="space-y-1">
          <h3 className="text-slate-400 text-xs font-semibold tracking-wider uppercase mb-1">{title}</h3>
